...
export default function PatternCanvas({ pattern, displayMode, highlightDmcId }: PatternCanvasProps) {
  ...
  // Transform state (CSS translate + scale)
  ...
  useEffect(() => {
    ...
    renderPattern(canvas, { ...pattern, grid: effectiveGrid }, { cellSize, showGrid, displayMode, highlightDmcId })
  ...
  // ── Transform helpers ─────────────────────────────────────────────────────────
  // Zoom around a point in container coords (focusX, focusY)
  ...
  // ── cellSize step buttons (렌더링 해상도) ─────────────────────────────────────
  const changeCellSize = useCallback((dir: 1 | -1) => {
    setCellSize(cur => {
  ...
  // ── Cell coordinate (getBoundingClientRect reflects CSS transform) ────────────
  ...
  const paintCell = useCallback((cx: number, cy: number, grid: number[][], erasing: boolean): number[][] => {
    ...
    if (erasing) {
      ...
      const next = grid.map(r => [...r]); next[cy][cx] = 0; return next
    }
    if (!selectedDmcId) return grid
    const targetIdx = pattern.dmcMap.findIndex(d => d.id === selectedDmcId)
    if (targetIdx < 0 || grid[cy][cx] === targetIdx) return grid
    const next = grid.map(r => [...r]); next[cy][cx] = targetIdx; return next
  ...
  const handleCanvasMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (editTool === 'eyedropper') {
      setSelectedDmcId(pattern.dmcMap[editableGrid[cy][cx]].id)
    ...
    if (editTool === 'paint' || editTool === 'erase') {
      ...
      const next = paintCell(cx, cy, editableGrid, editTool === 'erase' || e.shiftKey)
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const next = paintCell(coord.cx, coord.cy, editableGrid, editTool === 'erase' || e.shiftKey)
    if (next !== editableGrid) setEditableGrid(next)
  ...
  return (
    <div className="flex flex-col bg-linen-card">
            ...
            >
              <div
                ...
                style={{ background: selectedDmcId
                  ? (pattern.dmcMap.find(d => d.id === selectedDmcId)?.hex ?? '#fff')
                  : '#fff' }}
              />
      ...
      >
        ...
        {/* Transform wrapper — canvas moves and scales here */}
        <div
          style={{
            position:        'absolute',
            top:             0,
...
function InfoChip({ label, value }: { label: string; value: string }) {
  return (
    <span>{label}: <strong className="text-warm-500 font-normal">{value}</strong></span>
  )
}
